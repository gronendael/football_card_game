...
- [scripts/simulation/zone_coverage_runner.gd](scripts/simulation/zone_coverage_runner.gd): **`cover_zone`** defender movement (monitor box ±2 rows / ±1 col, side-based drift with anchor clamp, nearest-defender receiver claims, pursue-after-completion / pursue-after-LOS-or-awareness-on-run); `receiver_zone_pressure_tier` for pass separation. Defense playbook assignments via **`PlaySimContext.defense_play_row`** (`get_play(defense_play_id)` in `game_scene`).
