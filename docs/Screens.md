...
-- Event log (timestamped lines; **per-play** `Team (play): ±tile rows toward goal` line first on scrimmage; then **turnover** / **turnover on downs** / **change of possession** lines where applicable; first downs in teal after those; **Field goal** explicit **good** (`#66ff00`) / **missed** (`#ff6666`) then missed-FG turnover line; **PUNT** lines include punt / return / net tile rows; **Touchback** line for punt-in-endzone, kickoff after TD, opening kickoff, halftime second half)
-- Score
