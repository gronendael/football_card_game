...
-- Field (LOS / first-down markers; **BallChip** `🏈` at LOS, tinted by possession team; view mirrors when you are Away so your offense always advances toward the top). **Field lineup markers** (`FieldLineupMarkers` on the field): when offense/defense play ids are visible to you, white rounded (offense) / red (defense) chips sit on the play’s formation tiles; labels use **roster display names** from [PlaySimContext.lineup_slots](../scripts/resolution/play_sim_context.gd) (not bare formation-role text). Same-tile horizontal fan-out; defense chips shifted one perspective row. **Tick sim playback** (Debug) uses separate moving chips from `tick_snapshots`.
